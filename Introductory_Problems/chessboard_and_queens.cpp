...
bool issafe(ll i, ll j, vector<string> v)
{
    // if(i<0 || i>=tp.size() || j<0 || j>=tp.size()){return 0;}
    for (ll p = 0; p < i; p++)
    {
        if (v[p][j] == 'Q')
        {
...
void help(ll &ans, vector<string> v, ll i)
{
    if (i == 8)
    {
        ans++;
