...
int help(string &word1,string &word2,int i,int j){
        ...
        if(word1[i]==word2[j]){
                return dp[i][j]=help(word1,word2,i-1,j-1);
        }
        else{
                int a=help(word1,word2,i,j-1);
                int b=help(word1,word2,i-1,j);
                int c=help(word1,word2,i-1,j-1);
                return dp[i][j]=1+min({a,b,c});
        }
}
int solve2(string word1, string word2) {
    memset(dp,-1,sizeof(dp));
    return help(word1,word2,word1.size()-1,word2.size()-1);
}
void solve()
{
     string a,b;
